...
class Attention(nn.Module):
	...
	def forward(self, x, padding_mask=None, attention_weights=None, real_batch_size=None):
		B, N, C = x.shape
		...
		if attention_weights is not None:
			attn = attn.masked_fill(attention_weights, float("-inf"))
		if padding_mask is not None:
			padding_mask = repeat(padding_mask, 'b n -> (b t) n', t=(B // real_batch_size))
...
class DividedTemporalAttentionWithPreNorm(nn.Module):
	...
	def forward(self, query, key=None, value=None, residual=None, return_attention=False, padding_mask=None, attention_weights=None, **kwargs):
		...
		attn_out, attn_weights = self.attn(query, real_batch_size=b, padding_mask=padding_mask, attention_weights=attention_weights)
		if return_attention:
			return attn_weights
...
class DividedSpatialAttentionWithPreNorm(nn.Module):
	...
	def forward(self, query, key=None, value=None, residual=None, return_attention=False, padding_mask=None, attention_weights=None, **kwargs):
		...
		#attn_out = self.attn(query, query, query)[0]
		#attn_out = rearrange(attn_out, 'n b d -> b n d')
		attn_out, attn_weights = self.attn(query, padding_mask=padding_mask, attention_weights=attention_weights)
		if return_attention:
			return attn_weights
...
class TransformerContainer(nn.Module):
	...
	def forward(self, x, temporal_mask=None, skeleton_mask=None, return_attention=False):
		layer_idx = 0
		for layer in self.layers:
			if layer_idx >= self.num_transformer_layers-1 and return_attention:
				x = layer(x, temporal_mask, skeleton_mask, return_attention=True)
			else:
				x = layer(x, temporal_mask, skeleton_mask)
			layer_idx += 1
		return x
...
class BasicTransformerBlock(nn.Module):
	...
	def forward(self, x, temporal_mask=None, skeleton_mask=None, return_attention=False):
		...
		for layer in self.attentions:
			attention_weights = skeleton_mask if isinstance(layer, DividedSpatialAttentionWithPreNorm) else None
			padding_mask = temporal_mask if isinstance(layer, DividedTemporalAttentionWithPreNorm) else None

			if attention_idx >= len(self.attentions)-1 and return_attention:
				x = layer(x, padding_mask=padding_mask, attention_weights=attention_weights, return_attention=True)
				return x
			else:
				x = layer(x, padding_mask=padding_mask, attention_weights=attention_weights)
			attention_idx += 1
